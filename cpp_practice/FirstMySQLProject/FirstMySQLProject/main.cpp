...
void database(string name) {
	...
	//delete stmt;
	pstmt = con->prepareStatement("INSERT INTO test1(id, password, name) VALUES(?,?,?)"); // INSERT
...
void add_client() {
	...
	recv(new_client.sck, id, MAX_SIZE, 0);
	recv(new_client.sck, password, MAX_SIZE, 0);
	recv(new_client.sck, name, MAX_SIZE, 0);
	database(id, password, name);
	cout << "[" << nickname << "]";
	ZeroMemory(nickname, MAX_SIZE); // addr의 메모리 영역을 0으로 초기화
